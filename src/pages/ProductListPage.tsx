...
import { fetchProducts, fetchCategories, setFilter } from '@/features/productListingSlice';
import type { Product } from '@/types';
import { Link, useSearchParams } from 'react-router-dom';

export default function ProductListPage() {
  const dispatch = useAppDispatch();
  const { products, categories, loading, error, filter } = useAppSelector((s) => s.productListing);
  ...
  return (
    <section>
      ...
      {loading && (
        <div role='status' className='py-8'>Loading products...</div>
      )}
      {error && (
